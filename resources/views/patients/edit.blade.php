...
<body class="bg-gray-50 antialiased" style="font-family: 'Instrument Sans', sans-serif;">
    ...
    @php
        ...
        $pFirst = $patient['firstName'] ?? '';
        $pMiddle = $patient['middleName'] ?? '';
        $pLast = $patient['lastName'] ?? '';
        $pFullName = $patient['fullName'] ?? trim("$pFirst $pLast");
        $pBirth = $patient['birthDate'] ?? '';
        ...
        $pDevice = implode(', ', $pDevices); // for display only
        // $patientID is the Firestore docID passed from the controller — do not overwrite it
        ...
        <div class="flex-1 flex flex-col min-h-screen lg:ml-60 overflow-hidden" style="margin-left:200px;"
            ...
            <main class="flex-1 overflow-y-auto p-6">
                ...
                <nav class="flex items-center gap-2 text-xs text-gray-400 mb-6">
                    <a href="{{ route('patients.index') }}" class="hover:text-gray-700 transition-colors">Patients</a>
                ...
                <div class="max-w-2xl">
                    ...
                    <form method="POST" action="{{ route('patients.update', $patientID) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="bg-white rounded-2xl border border-gray-100 p-6 mb-5">
                            <div class="flex items-center gap-4">
                                ...
                                <div>
                                    <p class="font-semibold text-gray-900 text-base">{{ $pFullName ?: 'Patient' }}</p>
                                    @if ($pBirth)
                                        <p class="text-sm text-gray-400">
                                            {{ \Carbon\Carbon::parse($pBirth)->format('M d, Y') }}
                                            &middot; {{ \Carbon\Carbon::parse($pBirth)->age }} years old
                                        </p>
                                    @endif
                                    <span
                                        class="inline-block mt-1 text-xs font-medium bg-red-50 text-red-500 px-2 py-0.5 rounded-full">
                                        Patient
                                    </span>
                                </div>
                            </div>
                        ...
                        <div class="bg-white rounded-2xl border border-gray-100 p-6 mb-5">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
                                <div>
                                    ...
                                    <input type="text" name="firstName" value="{{ old('firstName', $pFirst) }}"
                                ...
                                <div>
                                    ...
                                    <input type="text" name="middleName"
                                        value="{{ old('middleName', $pMiddle) }}" placeholder="e.g. Santos"
                                ...
                                <div>
                                    ...
                                    <input type="text" name="lastName" value="{{ old('lastName', $pLast) }}"
                            ...
                            <div>
                                ...
                                <input type="date" name="birthDate" value="{{ old('birthDate', $pBirth) }}"
                                    class="w-full px-3 py-2.5 text-sm border rounded-xl bg-gray-50
                                       focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition
                                       @error('birthDate') border-red-300 @else border-gray-200 @enderror">
                                @error('birthDate')
                                    <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <textarea name="address" rows="2" placeholder="Street, Barangay, City, Province"
                                        class="w-full px-3 py-2.5 text-sm border border-gray-200 rounded-xl bg-gray-50
                                           focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition resize-none">{{ old('address', $pAddress) }}</textarea>
                                </div>
                                <div>
                                    <label class="block text-xs font-medium text-gray-700 mb-1.5">
                                        CCTV Device IDs
                                    ...
                                    <div id="deviceList" class="space-y-2">
                                        @php
                                            $editDevices = old('deviceID', $pDevices);
                                            if (!is_array($editDevices)) $editDevices = [$editDevices];
                                            if (empty($editDevices)) $editDevices = [''];
                                        @endphp
                                        @foreach ($editDevices as $did)
                                            <div class="flex items-center gap-2 device-row">
                                                ...
                                                <button type="button" onclick="removeDeviceRow(this)"
                                                    class="remove-btn text-gray-300 hover:text-red-500 transition-colors {{ count($editDevices) === 1 ? 'hidden' : '' }}">
