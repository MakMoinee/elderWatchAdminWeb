...
<body class="bg-gray-50 antialiased" style="font-family: 'Instrument Sans', sans-serif;">
            ...
            <main class="flex-1 overflow-y-auto p-6 space-y-5">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                        ...
                        <p class="text-sm text-gray-500 mt-0.5">
                            {{ $totalPatients }} patient{{ $totalPatients !== 1 ? 's' : '' }} under monitoring
                        </p>
                    </div>
                ...
                <div class="bg-white rounded-2xl border border-gray-100 px-4 py-3 flex flex-col sm:flex-row gap-3">
                    <div class="relative flex-1">
                        ...
                        <input id="searchInput" type="text" placeholder="Search by name or device ID…"
                            oninput="filterTable()"
                    ...
                    <div class="text-sm text-gray-400 flex items-center shrink-0">
                        <span
                            id="visibleCount">{{ $totalPatients }}</span>&nbsp;result{{ $totalPatients !== 1 ? 's' : '' }}
                    </div>
                </div>
                    ...
                    @if ($totalPatients > 0)
                                ...
                                <tbody class="divide-y divide-gray-50">
                                    @foreach ($patients as $patient)
                                        @php
                                            ...
                                            $pFullName = trim("$pFirst $pMiddle $pLast");
                                            $pDisplay = $patient['fullName'] ?? $pFullName;
                                            $pInitial = strtoupper(substr($pFirst, 0, 1)) ?: 'P';
                                        ...
                                        <tr class="hover:bg-gray-50/50 transition-colors patient-row"
                                            data-name="{{ strtolower($pDisplay) }}"
                                            data-device="{{ strtolower($pDevice) }}">
                                            ...
                                            <td class="px-6 py-4">
                                                <div class="flex items-center gap-3">
                                                    <div
                                                        ...
                                                        <span
                                                            class="text-sm font-semibold text-red-500">{{ $pInitial }}</span>
                                                    </div>
                                                    <div>
                                                        <p class="font-medium text-gray-900">{{ $pDisplay ?: '—' }}
                                                        </p>
                                            ...
                                            <td class="px-6 py-4 text-gray-600">
                                                {{ $pBirth ? \Carbon\Carbon::parse($pBirth)->format('M d, Y') : '—' }}
                                            </td>
                                            ...
                                            <td class="px-6 py-4">
                                                @if ($pAge !== '—')
                                                    <span
                                                        class="inline-block text-xs font-semibold px-2.5 py-1 rounded-full bg-red-50 text-red-600">
                                                        {{ $pAge }} yrs
                                                    </span>
                                                @else
                                            ...
                                            <td class="px-6 py-4 text-right">
                                                <div class="flex items-center justify-end gap-2">
                                                    <a href="{{ route('patients.edit', $pDocId) }}"
                                                    ...
                                                    <button
                                                        onclick="confirmDelete('{{ $pDocId }}', '{{ addslashes($pDisplay) }}')"
                        ...
                        <div class="px-6 py-3 border-t border-gray-50">
                            <p class="text-xs text-gray-400">
                                Showing <span id="shownCount">{{ $totalPatients }}</span> of {{ $totalPatients }}
                                patients
                            </p>
                        </div>
                    @else
                        <div class="flex flex-col items-center justify-center py-20 text-center px-6">
                            <div class="w-16 h-16 rounded-2xl bg-red-50 flex items-center justify-center mb-4">
                                <svg class="w-8 h-8 text-red-300" fill="none" viewBox="0 0 24 24"
                                    stroke="currentColor" stroke-width="1.5">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                                        d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                                </svg>
                            </div>
                            <h3 class="text-sm font-semibold text-gray-900 mb-1">No patients yet</h3>
                            <p class="text-sm text-gray-400 mb-6">Get started by registering the first patient.</p>
                            <a href="{{ route('patients.create') }}"
                                class="inline-flex items-center gap-2 bg-blue-600 text-white text-sm font-semibold
    ...
    <script>
        ...
        function filterTable() {
            ...
            let visible = 0;
            rows.forEach(row => {
                const match = row.dataset.name.includes(q) || row.dataset.device.includes(q);
