...
<body class="bg-gray-50 antialiased" style="font-family: 'Instrument Sans', sans-serif;">
        ...
        <div class="flex-1 flex flex-col min-h-screen lg:ml-60 overflow-hidden" style="margin-left:200px;">
            ...
            <main class="flex-1 p-6">
                <div class="max-w-3xl mx-auto space-y-6">
                    ...
                    <div
                        class="bg-gradient-to-r from-gray-900 to-gray-800 rounded-xl p-5 flex items-center gap-4 shadow">
                        <div
                            class="w-14 h-14 rounded-xl bg-yellow-400/20 flex items-center justify-center flex-shrink-0">
                        ...
                        <div class="flex-1 min-w-0">
                            <p class="text-blue font-bold text-lg">{{ $deviceDocID }}</p>
                            <p class="text-gray-400 text-sm font-mono">{{ $ip }}</p>
                        </div>
                        <div class="flex-shrink-0">
                            @if ($status === 'online')
                                <span
                                    class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium bg-green-500/20 text-green-400">
                                    <span class="w-1.5 h-1.5 rounded-full bg-green-400 animate-pulse"></span>
                                    Online
                                </span>
                            @else
                                <span
                                    class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium bg-gray-500/20 text-gray-400">
                    ...
                    <form action="{{ route('cctv.update', $deviceDocID) }}" method="POST" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                            ...
                            <div class="px-6 py-5 space-y-4">
                                ...
                                <div>
                                    ...
                                    <div
                                        class="flex items-center gap-2 px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-lg">
                                        <span class="text-sm text-gray-600 font-mono">{{ $deviceDocID }}</span>
                                        <span
                                            class="ml-auto text-xs text-gray-400 bg-gray-200 px-2 py-0.5 rounded">Read-only</span>
                                    </div>
                                </div>
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input type="text" name="ip" value="{{ old('ip', $ip) }}"
                                    ...
                                    <div>
                                        ...
                                        <select name="status"
                                            class="w-full px-3.5 py-2.5 text-sm border border-gray-300 rounded-lg
                                               focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                                            <option value="online"
                                                {{ old('status', $status) === 'online' ? 'selected' : '' }}>Online
                                            </option>
                                            <option value="offline"
                                                {{ old('status', $status) === 'offline' ? 'selected' : '' }}>Offline
                                            </option>
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="border border-gray-200 rounded-lg overflow-hidden">
                                    <button type="button" id="togglePasswordBtn" onclick="togglePasswordSection()"
                                        ...
                                        <svg id="toggleChevron" class="w-4 h-4 text-gray-400 transition-transform"
                                            fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M19 9l-7 7-7-7" />
                                        </svg>
                                    </button>
                                    <div id="passwordSection" class="hidden px-4 pb-4 pt-2 border-t border-gray-100">
                                        <p class="text-xs text-gray-500 mb-3">Leave blank to keep the current password.
                                        </p>
                                        <div class="relative">
                                            ...
                                            <button type="button"
                                                ...
                                                <svg id="eyeIcon" class="w-4 h-4" fill="none"
                                                    stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
    ...
    <script>
        ...
        const storedPass = @json($password);
        ...
        function updateRtspPreview() {
            ...
            const newPass = document.getElementById('passwordField') && document.getElementById('passwordField').value;
            const pass = newPass || storedPass || 'password';
        ...
        if (passField) passField.addEventListener('input', updateRtspPreview);
    </script>
