...
<head>
    ...
    <title>CCTV Devices — ElderWatch</title>
...
<body class="bg-gray-50 antialiased" style="font-family: 'Instrument Sans', sans-serif;">
        ...
        <div class="flex-1 flex flex-col min-h-screen lg:ml-60 overflow-hidden" style="margin-left:200px;">
            ...
            <main class="flex-1 p-6 space-y-6">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                        <h2 class="text-xl font-bold text-gray-900">Devices</h2>
                    </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4" style="margin-top:20px;">
                    ...
                    <div class="bg-white rounded-xl p-4 border border-gray-200 flex items-center gap-4">
                        ...
                        <div>
                            <p class="text-2xl font-bold text-gray-900">{{ $offline }}</p>
                ...
                <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden"
                    ...
                    <div
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-xs text-gray-500 whitespace-nowrap">
                                <span id="rowCount">{{ $totalDevices }}</span> device(s)
                            </span>
                        </div>
                    </div>
                    ...
                    <div class="overflow-x-auto">
                                ...
                                @forelse ($devices as $device)
                                    ...
                                    <tr class="hover:bg-gray-50 transition-colors device-row"
                                        ...
                                        <td class="px-5 py-3">
                                            <div class="flex items-center gap-3">
                                                <div
                                                    class="w-9 h-9 rounded-lg bg-yellow-100 flex items-center justify-center flex-shrink-0">
                                                ...
                                                <div>
                                                    <p class="font-semibold text-gray-900">{{ $deviceID }}</p>
                                        ...
                                        {{-- IP --}}
                                        <td class="px-5 py-3 text-gray-700 font-mono text-xs">{{ $ip }}</td>
                                        ...
                                        <td class="px-5 py-3">
                                            <div class="flex items-center gap-1.5">
                                                <code
                                                    class="text-xs bg-gray-100 text-gray-700 px-2 py-1 rounded font-mono max-w-[180px] truncate block"
                                                    title="{{ $rtspUrl }}">
                                                    rtsp://{{ $username }}:***@{{ $ip }}/stream
                                                </code>
                                                <button type="button"
                                                    onclick="copyRtsp('{{ addslashes($rtspUrl) }}', this)"
                                                    title="Copy RTSP URL"
                                                ...
                                                <button type="button"
                                                    onclick="showStreamModal('{{ addslashes($deviceID) }}', '{{ addslashes($rtspUrl) }}', '{{ addslashes($ip) }}', '{{ addslashes($username) }}')"
                                                    title="View stream info"
                                                    class="text-gray-400 hover:text-yellow-500 transition-colors flex-shrink-0">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                        viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
                                                            stroke-width="2"
                                        ...
                                        <td class="px-5 py-3">
                                            <div class="flex items-center justify-center gap-2">
                                                ...
                                                <button type="button"
                                                    onclick="confirmDelete('{{ $docID }}', '{{ addslashes($deviceID) }}')"
                                    ...
                                    <tr id="emptyRow">
                                        <td colspan="6" class="px-5 py-16 text-center">
                                            <div class="flex flex-col items-center gap-3">
                                                ...
                                                <a href="{{ route('cctv.create') }}"
                                                    class="text-sm text-blue-600 hover:underline">Add your first
                                                    device</a>
                                            </div>
                                        </td>
                                    </tr>
                        ...
                        <div id="noResults" class="hidden px-5 py-10 text-center">
                            <p class="text-gray-400 text-sm">No devices match your search.</p>
                        </div>
                    </div>
                </div>
            </main>
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
            ...
            <div class="bg-gradient-to-r from-gray-900 to-gray-800 px-6 py-4 flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p id="modalDeviceName" class="text-white font-semibold text-sm">Device</p>
                        <p class="text-gray-400 text-xs">Live Feed Information</p>
                    </div>
                </div>
                <button onclick="closeStreamModal()" class="text-gray-400 hover:text-white transition-colors">
            ...
            <div class="px-6 py-5 space-y-4">
                ...
                <div class="flex gap-3 bg-amber-50 border border-amber-200 rounded-lg p-3">
                    ...
                    <p class="text-xs text-amber-700">
                        RTSP streams require a compatible player (e.g. VLC Media Player). Browsers cannot play RTSP
                        natively.
                    </p>
                </div>
                ...
                {{-- RTSP URL --}}
                <div>
                    <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">RTSP
                        Stream URL</label>
                    <div class="flex items-center gap-2">
                        <code id="modalRtspUrl"
                            class="flex-1 bg-gray-900 text-green-400 text-xs px-3 py-2.5 rounded-lg font-mono break-all select-all">
                        </code>
                        <button onclick="copyModalRtsp()"
                            class="flex-shrink-0 bg-blue-600 hover:bg-blue-700 text-white text-xs px-3 py-2.5 rounded-lg font-medium transition-colors">
                            Copy
                        </button>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div class="bg-gray-50 rounded-lg p-3">
                        ...
                        <p id="modalIp" class="text-sm font-semibold text-gray-900 font-mono"></p>
                    </div>
                    <div class="bg-gray-50 rounded-lg p-3">
                        <p class="text-xs text-gray-500 mb-0.5">Username</p>
                        <p id="modalUsername" class="text-sm font-semibold text-gray-900"></p>
                    </div>
                </div>

                {{-- Open in VLC --}}
                <div class="bg-gray-900 rounded-lg p-4 flex items-center justify-between gap-3">
                    <div>
                        <p class="text-white text-sm font-medium">Open in VLC</p>
                        <p class="text-gray-400 text-xs mt-0.5">Use the RTSP URL above in Media → Open Network Stream
                        </p>
                    </div>
                    <a id="vlcLink" href="#"
                        class="flex-shrink-0 bg-orange-500 hover:bg-orange-600 text-white text-xs px-3 py-2 rounded-lg font-medium transition-colors"
                        target="_blank">
                        Open VLC
                    </a>
                </div>
            ...
            <div class="px-6 pb-5">
                <button onclick="closeStreamModal()"
                    class="w-full bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm font-medium py-2.5 rounded-lg transition-colors">
                    Close
                </button>
            </div>
        </div>
    ...
    <script>
        ...
        // ── Stream modal ─────────────────────────────────────────────────────────────
        function showStreamModal(deviceID, rtspUrl, ip, username) {
            document.getElementById('modalDeviceName').textContent = deviceID;
            document.getElementById('modalRtspUrl').textContent = rtspUrl;
            document.getElementById('modalIp').textContent = ip;
            document.getElementById('modalUsername').textContent = username;
            document.getElementById('vlcLink').href = rtspUrl;
            document.getElementById('streamModal').classList.remove('hidden');
            document.getElementById('streamModal').classList.add('flex');
        }

        function closeStreamModal() {
            document.getElementById('streamModal').classList.add('hidden');
            document.getElementById('streamModal').classList.remove('flex');
        }

        function copyModalRtsp() {
            const url = document.getElementById('modalRtspUrl').textContent.trim();
            navigator.clipboard.writeText(url).then(() => {
                Swal.fire({
                    toast: true,
                    position: 'top-end',
