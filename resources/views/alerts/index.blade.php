...
@php
    ...
    $navItems = [
        ...
        ['label' => 'Guardians',    'route' => 'guardians.index',  'active' => false, 'icon' => 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z'],
        ...
        ['label' => 'Schedule',     'route' => 'activities.index', 'active' => false, 'icon' => 'M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z'],
        ...
        ['label' => 'Alerts',       'route' => 'alerts.index',     'active' => true,  'icon' => 'M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9'],
    ];
@endphp
...
<div class="flex h-screen overflow-hidden">
    ...
    {{-- SIDEBAR --}}
    <aside id="sidebar" class="fixed inset-y-0 left-0 z-40 w-60 bg-white border-r border-gray-100 flex flex-col transform -translate-x-full lg:translate-x-0 transition-transform duration-200 ease-in-out">
        ...
        <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-0.5">
            @foreach ($navItems as $item)
                ...
                <a href="{{ $href }}" class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors {{ $item['active'] ? 'bg-blue-50 text-blue-700' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900' }}">
                    <svg class="w-5 h-5 shrink-0 {{ $item['active'] ? 'text-blue-600' : 'text-gray-400' }}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
    ...
    <div id="sidebar-overlay" class="fixed inset-0 z-30 bg-black/20 backdrop-blur-sm hidden lg:hidden" onclick="toggleSidebar()"></div>
        ...
        <main class="flex-1 overflow-y-auto p-6 space-y-5">
            ...
            <div class="bg-white rounded-2xl border border-gray-100 px-4 py-3 flex flex-col sm:flex-row gap-3">
                <div class="relative flex-1">
                    ...
                    <input id="searchInput" type="text" placeholder="Search by caregiver or IP…" oninput="filterTable()"
                ...
                <div class="text-sm text-gray-400 flex items-center shrink-0">
                    <span id="visibleCount">{{ $totalAlerts }}</span>&nbsp;event{{ $totalAlerts !== 1 ? 's' : '' }}
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl border border-gray-100 overflow-hidden">
                @if ($totalAlerts > 0)
                        ...
                        <table class="w-full text-sm">
                            <thead>
                                <tr class="border-b border-gray-100 bg-gray-50/60">
                                    ...
                                    <th class="text-left text-xs font-semibold text-gray-500 uppercase tracking-wide px-6 py-3">Caregiver</th>
                                    <th class="text-left text-xs font-semibold text-gray-500 uppercase tracking-wide px-6 py-3">IP Address</th>
                                    <th class="text-left text-xs font-semibold text-gray-500 uppercase tracking-wide px-6 py-3">Timestamp</th>
                                    <th class="text-left text-xs font-semibold text-gray-500 uppercase tracking-wide px-6 py-3">Image</th>
                                </tr>
                            </thead>
                                ...
                                @foreach ($alerts as $alert)
                                    @php
                                        ...
                                        if ($rawTs instanceof \Google\Cloud\Core\Timestamp) {
                                            $timestamp = \Carbon\Carbon::createFromTimestamp($rawTs->get()->getTimestamp());
                                        } elseif ($rawTs) {
                                            try { $timestamp = \Carbon\Carbon::parse((string) $rawTs); }
                                        ...
                                        $colors = $statusColors[$status] ?? ['bg' => 'bg-gray-100', 'text' => 'text-gray-600', 'dot' => 'bg-gray-400'];
                                    @endphp
                                    <tr class="hover:bg-gray-50/50 transition-colors alert-row"
                                        ...
                                        <td class="px-6 py-4">
                                            <span class="inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-full {{ $colors['bg'] }} {{ $colors['text'] }}">
                                                <span class="w-1.5 h-1.5 rounded-full {{ $colors['dot'] }} {{ $status === 'alert' ? 'animate-pulse' : '' }}"></span>
                                                {{ ucfirst($status) }}
                                            </span>
                                        </td>
                                        ...
                                        <td class="px-6 py-4">
                                            <div class="flex items-center gap-2.5">
                                                <div class="w-8 h-8 rounded-full bg-blue-50 flex items-center justify-center shrink-0">
                                                    <span class="text-xs font-semibold text-blue-600">{{ $cgInitial }}</span>
                                                </div>
                                                <span class="font-medium text-gray-900">{{ $caregiverName }}</span>
                                            </div>
                                        </td>

                                        {{-- IP --}}
                                        <td class="px-6 py-4 font-mono text-xs text-gray-600">{{ $ip }}</td>
                    ...
                    <div class="px-6 py-3 border-t border-gray-50">
                        <p class="text-xs text-gray-400">Showing <span id="shownCount">{{ $totalAlerts }}</span> of {{ $totalAlerts }} events</p>
                    </div>
                @else
                    <div class="flex flex-col items-center justify-center py-20 text-center px-6">
                        ...
                        <p class="text-sm text-gray-400">Activity events will appear here when caregivers check in.</p>
                    </div>
                @endif
            </div>
        </main>
...
<div id="imageModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/70 backdrop-blur-sm p-4">
    <div class="bg-white rounded-2xl shadow-2xl overflow-hidden max-w-lg w-full">
        ...
        <div class="p-4">
            <img id="modalImage" src="" alt="Activity capture" class="w-full rounded-xl object-cover max-h-80"
