...
<head>
    ...
    <title>Add Schedule — ElderWatch</title>
...
@php
    ...
    $navItems = [
        ...
        ['label' => 'Schedule',     'route' => 'activities.index', 'active' => true,  'icon' => 'M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z'],
    ...
    <div class="flex-1 flex flex-col min-h-screen lg:ml-60 overflow-hidden" style="margin-left:200px;">
        <header class="h-16 bg-white border-b border-gray-100 flex items-center justify-between px-6 shrink-0 z-20">
            ...
            <div class="flex items-center gap-3 flex-1">
                ...
                <div>
                    <h1 class="text-sm font-semibold text-gray-900">Add Schedule</h1>
                    <p class="text-xs text-gray-400">Assign a caregiver to a patient</p>
                </div>
            </div>
        ...
        <main class="flex-1 overflow-y-auto p-6">
            <form action="{{ route('activities.store') }}" method="POST" class="max-w-xl mx-auto space-y-5">
                @csrf
                ...
                <div class="bg-white rounded-2xl border border-gray-100 overflow-hidden">
                    <div class="px-6 py-4 border-b border-gray-100">
                        ...
                        <p class="text-xs text-gray-400 mt-0.5">Choose the caregiver, patient, and schedule time</p>
                    </div>
                    <div class="px-6 py-5 space-y-4">
                        ...
                        <div>
                            ...
                            <select name="caregiverID"
                                class="w-full px-3.5 py-2.5 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('caregiverID') border-red-400 @enderror">
                                <option value="">— Select caregiver —</option>
                                @foreach ($caregivers as $c)
                                    @php $cName = trim(($c['firstName'] ?? '') . ' ' . ($c['lastName'] ?? '')); @endphp
                                    <option value="{{ $c['docID'] }}" {{ old('caregiverID') === $c['docID'] ? 'selected' : '' }}>
                                        {{ $cName }}
                                    </option>
                                @endforeach
                            </select>
                            @error('caregiverID')<p class="text-xs text-red-500 mt-1">{{ $message }}</p>@enderror
                        </div>
                        ...
                        <div>
                            ...
                            <select name="patientID"
                                class="w-full px-3.5 py-2.5 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('patientID') border-red-400 @enderror">
                                <option value="">— Select patient —</option>
                                @foreach ($patients as $p)
                                    <option value="{{ $p['docID'] }}" {{ old('patientID') === $p['docID'] ? 'selected' : '' }}>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="date" name="date" value="{{ old('date') }}"
                                    class="w-full px-3.5 py-2.5 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('date') border-red-400 @enderror" />
                            ...
                            <div>
                                ...
                                <input type="time" name="time" value="{{ old('time') }}"
                                    class="w-full px-3.5 py-2.5 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('time') border-red-400 @enderror" />
